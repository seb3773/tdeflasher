...
# ==============================================================================
# tdeFlasher - Q4OS Installer (.qsi) Builder
# ==============================================================================
...
echo "=================================================="
echo " tdeFlasher - Q4OS .qsi Package Builder"
echo "=================================================="
...
find_latest_deb() {
    local debs=("$SCRIPT_DIR"/tdeflasher_[0-9]*_amd64.deb "$SCRIPT_DIR"/*.deb)
    for f in "${debs[@]}"; do
        if [ -f "$f" ] && [[ "$f" != *"_static_"* ]]; then
            echo "$f"
            return 0
        fi
    done
...
PACKAGE_NAME=$(dpkg-deb -f "$LATEST_DEB" Package 2>/dev/null || echo "tdeflasher")
APP_VERSION=$(dpkg-deb -f "$LATEST_DEB" Version 2>/dev/null || echo "1.0.0")
...
APPNAME_DESC="TDE-Flasher OS Image Writer"
APP_ICON="tdeflasher"
PK_VERS="$APP_VERSION"
SETUP_TYPE="2"
